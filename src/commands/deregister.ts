...
const handler = Effect.fn('Deregister.run')(function*({ path: input }) {
	...
	yield* persistedAppData.deregister(repoPath);
	const jsonl = yield* Output.Jsonl;
	yield* Console.log(
		Output.formatRegistrationResult('deregister', repoPath, jsonl)
	);
});

/** Deregister a repo directory. */
export const command = Command.make(
	'deregister',
	{ path: Argument.directory('path').pipe(Argument.optional) },
	handler
).pipe(
	Command.withDescription('Deregister a repo directory'),
